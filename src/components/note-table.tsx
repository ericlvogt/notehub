...
export default function NoteTable({
  ...
  return (
    <div className="inline-block overflow-hidden rounded-lg border shadow-2xl">
      <table>
        ...
        <tbody>
          {data?.map((note) => (
            <tr
              key={note.id}
              className="divide-x hover:cursor-pointer hover:bg-notehub-highlightedLight hover:dark:bg-notehub-highlightedDark"
              onClick={() =>
                router.push(`/${note.user.name as string}/${note.name}`)
              }
            >
