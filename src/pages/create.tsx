...
const Create: NextPage = () => {
  ...
  const persistSubmit = async () => {
    ...
    if (
      ...
      schools.data[0]?.name === schoolSearchTerm
    ) {
      school = schools.data[0];
    } else {
      school = await createSchoolMutation.mutateAsync({
        name: schoolSearchTerm,
      });
    }
    ...
    if (
      ...
      courses.data[0]?.name === courseSearchTerm
    ) {
      course = courses.data[0];
    } else {
      course = await createCourseMutation.mutateAsync({
        name: courseSearchTerm,
        schoolId: school.id,
      });
    }
    ...
    <>
      <Layout>
        <div className="flex flex-col items-center justify-items-center">
          <h1 className="pt-10 pb-5 text-5xl font-bold">Create</h1>
            ...
            <div className="flex flex-col gap-y-2">
              <div className="flex flex-row items-end">
                <div className="flex flex-col">
                  ...
                  <span
                    id="creator"
                    className="pointer-events-none whitespace-nowrap rounded border p-2"
                  >
                    {sessionData.user?.name}
                  </span>
                </div>
                <span className="whitespace-nowrap p-2 px-2 ">/</span>
                <div>
                  <label htmlFor="repository">Repository</label>
                  <input
                    id="repository"
                    className="w-full rounded border bg-inherit p-2 text-inherit"
                    type="text"
                    placeholder="Name"
                    value={name}
              ...
              <div>
                ...
                <DropdownSearch
                  items={schools.data?.map(
                    (x) => ({ name: x.name } as DropdownItem)
                  )}
                  placeholder="School"
              ...
              <button
                className="mt-2 rounded bg-notehub-secondary p-2 text-notehub-light"
                type="submit"
              >
                Submit
              </button>
